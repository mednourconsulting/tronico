...
@Component({
  selector: 'ngx-atelier-chart',
  templateUrl: './atelier-chart.component.html',
  styleUrls: ['./atelier-chart.component.scss'],
providers: [DashboardFragService, OtdWeekService, PcdWeekService, EvolutionheuresService, TrsCMSService, UsersService ],
})
  export class AtelierChartComponent implements OnInit {
  isAdmin: boolean;
  type = 'month';
  ...
  constructor(private dashboardFragService: DashboardFragService,
    ...
    this.atelier = this.router.getCurrentNavigation().extras.queryParams.data;
  ...
  searchForOtd() {
    if (this.weekFirst.weekFirstOtd < this.weekFin.weekFinOtd) {
      this.serviceOtd.getAll(this.atelier, this.year.yearOtd).subscribe(data => {
  ...
  searchForPcd() {
    if (this.weekFirst.weekFirstsPcd < this.weekFin.weekFinPcd) {
      this.servicePcd.getAll(this.atelier, this.year.yearPcd).subscribe(data => {
        if (Object.keys(data).length === 0) {
          this.toastr.info('il n\'y a aucun d\'information au les semaines specifie ', 'Info');
        } else {
